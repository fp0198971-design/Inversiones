...
const SimplePieChart = ({ data, colors }) => {
  ...
  return (
    <div className="relative h-64 w-64 mx-auto">
      ...
      {/* Centro blanco para efecto Donut (Opcional, quitar para Pie completo) */}
...
export default function App() {
  ...
  const financialHealth = useMemo(() => {
    // Estimación simple de deuda basada en categorías específicas (ejemplo)
    const debtPayment = transactions
      .filter(t => t.type === 'expense' && (t.category === 'Deuda' || t.category === 'Créditos'))
      .reduce((acc, t) => acc + t.amount, 0);
    
    const debtToIncome = summary.income > 0 ? (debtPayment / summary.income) * 100 : 0;
    ...
    const monthsOfRunway = monthlyExpenses > 0 ? (summary.balance / monthlyExpenses) : 0; // Usando balance acumulado como "ahorro líquido"

    return { debtToIncome, monthsOfRunway, monthlyExpenses };
  }, [summary, transactions]);

  const projectionData = useMemo(() => {
    const rate = riskProfiles[riskProfile].returnRate;
    const monthlyContribution = summary.balance > 0 ? summary.balance : 0; // Asumimos que todo el balance se invierte
    const years = 10;
    const data = [];
    let currentWealth = 0;
    ...
    for (let i = 0; i <= years; i++) {
      ...
      // Interés compuesto anual simple + contribución mensual * 12
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="border-l-4 border-indigo-500">
          ...
          <div className="mt-2 text-xs text-indigo-600 font-medium">+ disponible para invertir</div>
        </Card>
            ...
            <div className="space-y-2 text-sm w-full sm:w-auto">
              {summary.pieData.map((d, i) => (
                <div key={i} className="flex items-center justify-between gap-4">
                  ...
                  <span className="font-bold text-slate-800">{Math.round((d.value / summary.expenses) * 100)}%</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <Card>
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">Objetivo recomendado: 20%</p>
            </div>
            ...
            <div className="p-4 bg-indigo-50 rounded-lg border border-indigo-100">
               ...
               <p className="text-sm text-indigo-800 mb-2">
                 Tienes cubierto <span className="font-bold">{financialHealth.monthsOfRunway.toFixed(1)} meses</span> de gastos básicos con tu balance actual.
               </p>
          ...
          <Card className="bg-slate-900 text-white border-none">
            ...
            <div className="space-y-4">
              ...
              <div className="pt-4 border-t border-slate-700">
                <span className="text-slate-400 text-xs">Contribución Mensual Actual</span>
                <p className="text-xl font-bold text-white">{formatCurrency(Math.max(0, summary.balance))}</p>
                <p className="text-xs text-slate-400 mt-1">*Basado en tu flujo de caja libre</p>
              </div>
            </div>
          </Card>
        </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      Enfocado en crecimiento a largo plazo. Incluye ETFs globales como S&P 500 o Total World. 
                      Mayor volatilidad, mayor retorno esperado.
                    </p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      Enfocado en preservación de capital y estabilidad. Incluye bonos gubernamentales (CETES/Treasuries) y deuda corporativa.
                    </p>
                  </div>
                </div>
              </div>
      ...
      <aside className="hidden lg:flex flex-col w-64 h-screen fixed bg-white border-r border-slate-200 px-4 py-6">
        ...
        <div className="mt-auto pt-6 border-t border-slate-100">
          <div className="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl p-4 text-white">
            ...
            <p className="text-xs opacity-90 mb-3">Fondo de emergencia completo</p>
